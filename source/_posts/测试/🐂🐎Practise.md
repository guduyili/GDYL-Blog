...
2. 添加/编辑/详情
   - 页面内容是否正确
   - 输入框引导语
   - 必填项：是否有红*，输入为空失焦或点保存。提示：该项为必填项
   - **字符类型：**数字，大小写字母，汉字，HTML标签对< input >,特殊意义字符串（Null，null，转义字符，以上字符类型组合，内容前中后包含空格，输入特殊字符（!@#$%^&*()+{}|:"<>?;'等，尤其是单引号），输入下划线）
   - 字符长度：小于，等于，大于，复制粘贴超过长度的字符（只能粘贴长度内的）
   - 唯一性校验
   - 失焦事件
